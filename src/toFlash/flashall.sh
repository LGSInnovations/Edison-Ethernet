#!/bin/bash
# Edited by plusk at LGS Innovations for the Edison Ethernet block project using Ubilinux with the Yocto kernel.

# Only allow root to run this script.
# added by pcl
if [[ $(/usr/bin/id -u) -ne 0 ]]; then
    echo "Not running as root. Please use sudo to run flashall.sh"
    exit 1
fi
